...
const VolunteerUser = asyncHandler(async (req, res) => {
  const { firstname, lastname, phonenumber, password, latitude, longitude, interests } = req.body;

  if (!firstname || !lastname || !phonenumber || !password || !latitude || !longitude || !interests) {
    throw new ApiError(400, "Please provide all required fields: first name, last name, phone number, password, latitude, longitude, and interests");
  }
  
  const existedVolunteer = await Volunteer.findOne({ phonenumber });
  if (existedVolunteer) {
    throw new ApiError(409, "Volunteer with this phone number is already registered");
  }
...
const updateLocation = asyncHandler(async (req, res) => {
  ...
  if (!phonenumber || !latitude || !longitude) {
    throw new ApiError(400, "Please provide phone number, latitude, and longitude");
  }
  
  const volunteer = await Volunteer.findOne({ phonenumber });
  if (!volunteer) {
    throw new ApiError(404, "Volunteer not found with the provided phone number");
  }
